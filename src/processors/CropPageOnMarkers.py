...
def validate_shape(shape, c):
    peri = cv2.arcLength(c, True)
    approx = cv2.approxPolyDP(c, epsilon=0.025 * peri, closed=True)
    match shape:
        case "rectangle":
            return validate_rect(approx)
        case "square": 
            return validate_square(approx)
    return False
...
class CropPageOnMarkers(ImagePreprocessor):
    def __init__(self, *args, **kwargs):
        ...
        self.shape = cropping_ops.get("shape")
        self.shapeArea = cropping_ops.get("shapeArea")
    ...
    def find_page(self, image, file_path):
        ...
        shapeCnts = []
        for c in cnts:
            if validate_shape(self.shape, c):
                shapeCnts.append(c)

        # Filter contours by area
        areaCnts = []
        for c in shapeCnts:
            if cv2.contourArea(c) >= self.shapeArea:
                areaCnts.append(c)

        # Find top left contour coordinate
        top = config.dimensions.processing_height
        left = config.dimensions.processing_width
        right = 0
        bottom = 0
        ...
        cv2.circle(image, (left, top), 8, (255, 255, 255), -1)
        cv2.circle(image, (right, top), 8, (255, 255, 255), -1)
        cv2.circle(image, (left, bottom), 8, (255, 255, 255), -1)
        cv2.circle(image, (right, bottom), 8, (255, 255, 255), -1)
        cv2.circle(edge, (left, top), 8, (255, 255, 255), -1)
        cv2.circle(edge, (right, top), 8, (255, 255, 255), -1)
        cv2.circle(edge, (left, bottom), 8, (255, 255, 255), -1)
        cv2.circle(edge, (right, bottom), 8, (255, 255, 255), -1)

        sheet = np.reshape([(left, top), (right, top), (left, bottom), (right, bottom)], (4, -1))
